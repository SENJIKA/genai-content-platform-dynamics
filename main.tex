...
Bellman方程，并用两种替代性规格刻画非平稳在线学习：高斯漂移状态下的内生测试
曝光，以及两题材马尔可夫热点切换下的探索---利用配置。
...
当期匹配改善收益的下降速度，并非
由技术进步机械推出。在线学习扩展进一步表明，新增项目和测试曝光共同决定反馈精度；
当热点持续漂移时，平台不能永久学完需求，测试政策因而依赖后验均值、精度与状态
持久性。两题材模型还显示，热点切换会同时提高刷新信息的必要性并缩短信息寿命，
因此更快切换未必单调增加探索，成熟政策也可能由单向转换变为状态依赖的循环调整。
\end{abstract}
...
\citep{xu2026feedback}。本文借鉴的是“行动改变信号精度和后验转移”这一共同结构，
而不是上述论文的产品、竞争或披露机制。在线优化文献以变化预算和动态遗憾度量
非平稳性的代价\citep{besbes2015nonstationary}；本文不采用遗憾最小化作为平台目标，
平台仍最大化折现利润。渐进行动学习还强调当期行动持续产生公共信息与等待期权的
区别\citep{laiho2025gradual}。由于本文项目按经营批次结算，在线扩展保留信息生成
机制，但不把不可逆采用或多年等待期权作为既有模型能够推出的结论。
...
有条件、可检验的结论，而不是由技术进步机械推出。在此基础上，本文把慢速积累的
推荐资本与快速变化的热点状态分开，并给出两种替代性在线学习规格。高斯规格令项目
进入与测试曝光共同决定信号精度；热点规格令两类题材按照马尔可夫链切换，并由平台
选择类别曝光。两者均把完整反馈历史压缩为后验状态，同时说明非平稳需求可能使
“成熟以后永久停止探索”不再成立。

下文首先建立期内行动顺序并求解终端期均衡，继而分析平台当期合同和目标进入规模的
可实施性；随后构造两期动态模型，推导动态边际进入楔子，并在无限期环境中证明政策
阶段转换；继而给出两种非平稳在线学习扩展；最后报告两期、无限期与信念状态数值均衡、稳健性检验
和可检验的经验预测。
...
基准模型中的$M_t$是慢速积累的结构性推荐资本：它表示平台的多模态表征、匹配和
冷启动能力。热点状态则回答另一个问题，即“这一批次什么题材更受欢迎”。即使平台
拥有很高的$M_t$，热点仍可能改变；反过来，一次热点判断正确也不等于平台永久提高了
通用推荐能力。为避免把两种学习重复计算，本节采用双时间尺度。外层批次中的
$(K_t,M_t)$仍由基准模型决定；在给定结构状态$(\bar K,\bar M)$的在线子问题中，
高频曝光和反馈被聚合为一次测试决策。该子问题对每个结构状态完整求解，并可作为
外层Bellman方程的条件价值。若直接同时更新$M$与热点信念，必须分别说明哪些反馈
提高通用学习技术、哪些反馈只更新当前需求判断，不能把同一信息收益记两次。

“在线学习”在这里不是关于神经网络是否逐次更新权重的工程断言。经济学上的必要
结构只有三步：平台行动改变当期收益与信号精度，反馈更新后验，后验又改变下一批次
行动。以下给出两个替代性规格。高斯规格适合把热点理解为连续漂移的总体匹配状态；
有限状态规格适合直接刻画两类题材之间的热点切换。二者不在同一个数值模型中叠加。
...
唯一有界连续价值函数和最优平稳马尔可夫政策。这里的平台按正确转移规律进行贝叶斯
过滤，不采用\citet{esponda2021misspecified}的模型误设均衡概念。
...
命题\ref{prop:贝叶斯精度阈值}是条件命题。后验均值会改变当期进入收益，测试曝光又
内生改变精度转移，所以$\Omega_\xi^G<0$必须由原始函数或数值政策核验，不能由
贝叶斯公式直接推出。尤其是，在悲观状态中牺牲已有匹配的机会成本较低，平台可能
反而更愿意试验；因此“精度低就一定多测试”也不是无条件结论。
...
热点切换速度$\lambda$对探索没有一般的单调符号。更快切换使旧后验更快失效，产生
“需要刷新”的力量；但也缩短本期信号可以改善未来决策的期限，降低信息资产的耐久性。
这正是变化环境试验中可能出现不同追踪区制的原因\citep{keller1999changing}。因此，
本文不预设“热点变化越快，测试越多”，而在数值部分报告两种力量的净效应。高斯规格
与热点规格共同说明：稳定环境下的精度阈值可以作为局部政策边界，但持续变化的内容
市场更可能表现为状态依赖、可逆的扶持与测试政策。
%%%
...
在线扩展采用前文的双时间尺度，而不是直接把$(K,M,\mu,\xi)$或$(K,M,p)$全部放入
同一个高维网格。具体而言，基准模型先决定慢速结构状态$(\bar K,\bar M)$；在线子问题
在该状态下完整求解进入、分成与测试曝光的无限期信念状态均衡。这样既保留热点反馈
对未来行动的动态价值，也不会把同一反馈同时记入$M'$和热点后验。高斯规格取
...
图\ref{fig:高斯在线学习}显示，测试曝光并不只是后验精度的函数。在悲观且低精度的
...
附近的$0.125$，在更高精度处降为零。因此数值结果支持的是“均值与精度共同决定
测试区”，而不是无条件的“越不确定越测试”。以固定随机种子模拟90个批次，平均测试
...
为$0.760$。这些数值是机制校准而不是对现实推荐算法权重的估计。
...
也决定相对题材信号的质量。其信念和政策随新冲击可逆调整，因而“结构资本成熟”
不等于“此后永远停止探索”。两个规格均表明，新增项目与测试曝光是替代或互补的
信息生产工具；更快热点切换同时提高刷新需要并缩短信号寿命，所以净效应必须由原语
和数据判定。贝叶斯更新与马尔可夫转移在这里提供可检验的状态演化，而不是保证某种
政策方向的数学装饰。
...
版权责任、平台自制内容以及用户对AI标签的偏好。在线数值模型还把慢速结构状态
条件化处理，高斯规格使用有限数值网格，热点规格只保留两个对称题材；这些选择适合
核验机制，却不能替代包含多题材、用户异质性和结构资本共同转移的估计模型。数值
部分求解的是一组经过数据矩约束的部分校准均衡，而不是结构估计；其中短视频类别
数据只约束多样性，不能识别现实AI短剧合同、推荐算法权重或福利参数。因而，本文最适合解释那些由平台集中分发、依赖推荐冷启动、存在标准化
